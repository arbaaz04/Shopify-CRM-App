...
/**
 * Deletes an existing Sendit configuration
 */
export const run: ActionRun = async ({ params, record, logger, api }) => {
  try {
    logger.info('Deleting Sendit config', { id: record.id });
    ...
    logger.info('Successfully deleted Sendit config', { id: recordToReturn.id });
    
    // Return the deleted record instead of a success object
    return recordToReturn;
  } catch (error) {
    logger.error("Error deleting Sendit config", { 
